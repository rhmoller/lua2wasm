...
| **Numbers**       | int + float subtypes with Lua-compliant promotion (`/` always float, `+ - *` keep int if both ints, `//` floor-div, `%` floor-mod, `^` always float, hex int `0xFF` / `0Xff`, hex float `0x1.8p3`, bitwise `& \| ~ << >>` and unary `~` on 64-bit ints with float-to-integer coercion)                            | —                                                                |
...
3. `_G` — a real global env table aliasing all module globals; would also let strict mode honour `global <const> *` as an opt-in.
4. `<const>` and `<close>` local attributes (the latter needs `__close` and to-be-closed scope tracking).
5. `xpcall`, `error(msg, level)`, `warn`; full `debug.*` would be its own milestone.
6. `os.{clock, time, date, difftime, getenv}`, `io.open` / `io.lines` / file handles.
7. `load` / `loadfile` / `require` — dynamic compilation; needs the compiler at runtime.
8. Coroutines — blocked on the WASM stack-switching proposal landing in browsers.
9. Source maps so DevTools can step from compiled WASM back into Lua.
10. `wasm-opt` step in the build pipeline (size + speed wins for the shipped `.wasm`).
