...
| **Standard lib**  | `print`, `error`, `pcall`, `assert`, `select`, `type`, `tostring`, `tonumber`, `ipairs`, `pairs`, `next`, `setmetatable`, `getmetatable`, `rawequal`, `rawlen`, `rawget`, `rawset`; `io.{write, read}`; `_VERSION`; `math.{floor, ceil, abs, sqrt, min, max, sin, cos, tan, asin, acos, atan, exp, log, pi, huge}`; `string.{len, sub, format, upper, lower, reverse, rep, byte, char}` (`format` covers `%s %d %x %g %f %e %%`, optional `.N`; `upper`/`lower` are ASCII); `table.{insert, remove, concat, unpack, pack, move, create}` | `_G`, more of `string` (`find, gmatch, gsub, match, pack, unpack`), `table.sort`, format flags/width, `os.*`, `io.open` |
