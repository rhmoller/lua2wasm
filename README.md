...
3. `<const>` and `<close>` local attributes (the latter needs `__close` and to-be-closed scope tracking).
4. `error(msg, level)` position prefix (currently the `level` arg is accepted but ignored); full `debug.*` would be its own milestone.
5. `os.{clock, time, date, difftime, getenv}`, `io.open` / `io.lines` / file handles.
6. `load` / `loadfile` / `require` — dynamic compilation; needs the compiler at runtime.
...
9. `wasm-opt` step in the build pipeline (size + speed wins for the shipped `.wasm`).
